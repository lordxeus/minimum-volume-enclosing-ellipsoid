...
void findPositive(gsl_vector* posIndices, const gsl_vector* x)
{
	...
	int n = x->size;
	for (i=0; i<n; i++) {
		if (gsl_vector_get(x,i)>VERYSMALL) {
...
void updateR(gsl_matrix* R,double* factor,gsl_vector* p,gsl_vector* z, double* tau, int* down_err)
{
	*factor = (*factor) / (1.0 - (*tau));
	...
	//gsl_vector* p = gsl_vector_alloc(z->size); gsl_vector_memcpy(p, z);
	//My_dtrsv(CblasLower, CblasNoTrans, CblasNonUnit, R, p);
